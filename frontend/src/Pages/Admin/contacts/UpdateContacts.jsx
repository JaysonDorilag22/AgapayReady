...
export default function UpdateContacts() {
  const { contactId } = useParams(); // Assuming you have a route parameter for guideline ID
  ...
  const fetchGuidelineData = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_PORT}/api/v1/contacts/${contactId}`);
...
console.log(formData.image);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`${import.meta.env.VITE_PORT}/api/v1/contacts/${contactId}`, formDataToSend, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });
      console.log("Contact updated:", response.data);
      navigate("/admin/create/contacts"); // Redirect to guidelines list after successful update
    } catch (error) {
  ...
  return (
    <div className="flex">
      ...
      <div className="flex-1 ml-64 mr-4 mt-8 p-4 rounded shadow">
        <h1 className="text-2xl font-bold mb-4">Update Contacts</h1>
